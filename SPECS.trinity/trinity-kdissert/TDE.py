...
def detect_kde(conf):
	...
	else: env['TDEDIR'] = exec_and_read('%s -prefix' % kde_config)

	print "Checking for kde version           :",
	kde_version = exec_and_read('%s --version|grep TDE' % kde_config).split()[1]
	p('GREEN',kde_version)
	...
	if qtdir:
		...
		try:
			...
			libtdeuiSO = os.path.join(tmplibdir, getSOfromLA(os.path.join(tmplibdir,'/libtdeui.la')) )
			m = re.search('(.*)/lib/libqt.*', exec_and_read('ldd %s | grep libqt' % libtdeuiSO).split()[2])
		except: m=None
		if m:
			qtdir = m.group(1)
	...
	if os.path.isfile(uic):
		...
		uic = exec_and_read("which tquic 2>/dev/null")
		if len(uic):
	...
	if os.path.isfile(moc):
		...
		moc = exec_and_read("which tqmoc 2>/dev/null")
		if len(moc):
	...
	if os.path.isfile(kdeincludes + "/klineedit.h"):
		...
		if os.path.isfile(kdeprefix+"/include/tde/klineedit.h"):
			...
			p('RED',"* Make sure tdelibs-dev is installed")
			p('RED',"* Try 'waf -h' for more options")
			sys.exit(1)
	...
	env['CPPPATH_TDECORE']=kdeincludes
	if not tdelibs:
		tdelibs=exec_and_read('%s --expandvars --install lib' % kde_config)
	env['LIBPATH_TDECORE']=tdelibs
	...
	try:
		if Params.g_options.want_rpath:
			...
			env['RPATH_TDECORE']=['-Wl,--rpath='+tdelibs]
	except:
		pass

	env['LIB_TDECORE']  = 'tdecore'
        env['LIB_TDEIO']      = 'tdeio'
        env['LIB_KMDI']     = 'tdemdi'
        env['LIB_TDEPARTS']   = 'tdeparts'
        env['LIB_TDEPRINT'] = 'tdeprint'
	env['LIB_TDEGAMES'] = 'tdegames'

	env['KCONFIG_COMPILER'] = 'tdeconfig_compiler'
...
def set_options(opt):
	...
	for i in "execprefix datadir libdir tdedir kdeincludes tdelibs qtdir qtincludes qtlibs libsuffix".split():
