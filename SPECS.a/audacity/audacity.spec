...
# Patch4: audacity-2.0.4-equalization-segfault.patch
...
Audacity 是一个跨平台的多音轨音频编辑器。它允许你直接录制声音或导入多种格式
的音频，它支持一些简单的物资，以及你应需要的编辑特性。GUI 是用 wxWigets 写的。

在 Linux 下支持的 I/O 包括 PulseAudio, OSS 和 ALSA。

%package manual
Summary: manual for Audacity - offline install
Summary(zh_CN.UTF-8): %{name} 的手册
BuildArch: noarch
