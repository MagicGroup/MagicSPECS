...
高级 Linux 声音架构 (ALSA) 库提供了在 Linux 操作上处理音频和 MIDI 的函数。

这个包包含了 ALSA 运行库以简化运行程序，并提供了旧的 OSS API 的高级函数，
可以二进制兼容大部分 OSS 程序。
...
%{_datadir}/alsa/
/lib/modprobe.d/dist-*
%{_docdir}/alsa-lib-%{version}/modprobe-dist-oss.conf
...
%changelog
* Sat Nov 07 2015 Liu Di <liudidi@gmail.com> - 1.0.29-3
- 为 Magic 3.0 重建
