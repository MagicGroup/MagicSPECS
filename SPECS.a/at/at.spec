...
at 和 batch 命令从标准输入或指定文件中读取命令，然后允许你在特定的时间
运行这些命令。batch 或在系统负载降到一级水平后执行全集。这两个命令都使
用用户 Shell。

如果你需要面向时间的作业控制，那就是需要安装 at 包。注意：如果你要的是
经常性工具，比如每天/周在同一时间重复执行，你应该安装 crontab 来代替。
