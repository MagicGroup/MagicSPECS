...
AsciiDoc 是一种文本文档格式，可以用来写短文档、文章、书籍和 UNIX 手册。
它可以转换到 HTML 和 DocBook 格式。

%package doc
Summary:  Additional documentation and examples for asciidoc
Summary(zh_CN.UTF-8): asciidoc 的附加文档和例子
...
Summary(zh_CN.UTF-8): asciidoc 的 latex 输出支持
Requires: %{name} = %{version}-%{release}
Requires: dblatex
...
asciidoc 的 latex 输出支持。

%package music
Summary:  Support for asciidoc music output
Summary(zh_CN.UTF-8): asciidoc 的音乐输出支持
Requires: %{name} = %{version}-%{release}
Requires: lilypond
...
%description music -l zh_CN.UTF-8
asciidoc 的音乐输出支持。
...
%exclude %{_docdir}/asciidoc/examples
%exclude %{_docdir}/asciidoc/doc
...
%changelog
* Sat Nov 07 2015 Liu Di <liudidi@gmail.com> - 8.6.9-7
- 为 Magic 3.0 重建
