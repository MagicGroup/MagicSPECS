...
Source: module-init-tools-%{version}.tar.gz
Source1: module-init-tools-%{version}.tar.gz.sign
Source2: weak-modules
Source3: depmod-dist.conf
...
BuildRequires: docbook-utils glibc-static zlib-devel zlib-static
...
# New configuration files we ship should go in /lib/modprobe.d
# in order to allow the local admin to customize /etc/modprobe.d
mkdir -p $RPM_BUILD_ROOT/lib
mkdir -p $RPM_BUILD_ROOT/lib/modprobe.d

install -m 755 %{SOURCE2} $RPM_BUILD_ROOT/sbin/weak-modules

mkdir -p $RPM_BUILD_ROOT/etc/depmod.d
install -m 644 %{SOURCE3} $RPM_BUILD_ROOT/etc/depmod.d/dist.conf
...
%changelog
* Wed Nov 16 2011 John W. Linville <linville@redhat.com> - 3.16-4
- Add "backports" directory to search line in depmod-dist.conf

* Wed Aug 31 2011 Jon Masters <jcm@redhat.com> - 3.16-3
- Remove legacy dist-alsa, dist-oss, and dist.conf files (#725841)
- Make sure we own /lib/modprobe.d and put future stuff in there

* Mon Jul 18 2011 Matthew Garrett <mjg@redhat.com> 3.16-2
- Don't search /usr/local for configuration (rhbz: #714023)

* Thu Jun  2 2011 Jon Masters <jcm@jonmasters.org> 3.16-1
- Update to latest upstream release
- Remove explicit documentation install hack

* Thu Jun  2 2011 Jon Masters <jcm@jonmasters.org> 3.14-1
- Update to latest upstream release

* Tue May 31 2011 Jon Masters <jcm@jonmasters.org> 3.13-3
- Use bz2 tarballs generated by the upstream release server

* Tue May 31 2011 Jon Masters <jcm@jonmasters.org> 3.13-1
- Rebuild with local cached upstream files while kernel.org is syncing

* Tue May 31 2011 Jon Masters <jcm@jonmasters.org> 3.13-0
- New upstream release
- Supports multiple config file directories
- Fix handling of Linux 3.x series kernels

* Fri Apr 15 2011 Jon Masters <jcm@jonmasters.org> 3.12-5
- Do not call (un)mount during binfmt_misc (un)load.

* Tue Feb 08 2011 Fedora Release Engineering <rel-eng@lists.fedoraproject.org> - 3.12-4
- Rebuilt for https://fedoraproject.org/wiki/Fedora_15_Mass_Rebuild

* Tue Dec  7 2010 Jon Masters <jcm@jonmasters.org> - 3.12-3
- Fix some build requires bits and the sources file.

* Tue Dec  7 2010 Jon Masters <jcm@jonmasters.org> - 3.12
- Update to latest upstream (compressed module problem a non-issue now).

* Wed Nov 25 2009 Jon Masters <jcm@jonmasters.org> - 3.11.1-2
- Trying out a patch to do directory blacklists in depmod.

* Wed Oct 21 2009 Jon Masters <jcm@jonmasters.org> - 3.11.1-1
- Rebuild with latest upstream. Add a patch for sequencer handling.
- Resolves: #505421.

* Thu Oct  1 2009 Jon Masters <jcm@jonmasters.org> - 3.10-1
