...
# tar cfj gyp-svn1636.tar.bz2 build/
Source1:	gyp-svn1636.tar.bz2
# Support proper soname
Patch0:		v8-3.17.6.14-mini-soname.patch
...
# Not sure why they forbid -O2.
sed -i "s|'-O2',|'-01',|g" build/common.gypi

PARSED_OPT_FLAGS=`echo \'$RPM_OPT_FLAGS -Wno-error=strict-overflow -Wno-error=unused-local-typedefs -Wno-unused-local-typedefs -Wno-unused-but-set-variable\'| sed "s/ /',/g" | sed "s/',/', '/g"`
sed -i "s|'-O3',|$PARSED_OPT_FLAGS,|g" build/common.gypi

%build
make %{archname}.release werror=no console=readline strictaliasing=off library=shared soname_version=%{sover} mini_soname=%{somajor} V=1
...
install -p out/%{archname}.release/lib.target/libv8.so.%{sover} %{buildroot}%{_libdir}
mkdir -p %{buildroot}%{_bindir}
install -p -m0755 out/%{archname}.release/d8 %{buildroot}%{_bindir}
...
* Wed May 29 2013 Tom Callaway <spot@fedoraproject.org> - 1:3.17.6.14-1
- chromium 27 needs this version

* Fri Mar 22 2013 Tom Callaway <spot@fedoraproject.org> - 1:3.15.11.17-1
- chromium 25 needs this version
